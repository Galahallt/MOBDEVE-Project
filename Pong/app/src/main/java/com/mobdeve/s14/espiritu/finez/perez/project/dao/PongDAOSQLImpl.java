...
public class PongDAOSQLImpl implements PongDAO {
    ...
    public boolean addScore(ScoreModel score) {
        ...
        cv.put(ScoreDatabase.SCORE_TIME, score.getTime());
    ...
    public ArrayList<ScoreModel> getAllUserScores(String user) {
        String query = "SELECT * FROM " + ScoreDatabase.SCORE_TABLE + " WHERE " + ScoreDatabase.SCORE_USERNAME + "=\"" + user + "\"" + " ORDER BY " + ScoreDatabase.SCORE_TIME + " DESC";
    ...
    @Override
    public int getUserHighestScore(String user) {
        String query = "SELECT MAX(" + ScoreDatabase.SCORE_SCORE + ") FROM " + ScoreDatabase.SCORE_TABLE + " WHERE " + ScoreDatabase.SCORE_USERNAME + "=\"" + user + "\"";
        Cursor cursor = scoreDatabase.getReadableDatabase().rawQuery(query, null);
    ...
    public ArrayList<ScoreModel> getAllUserHighestScore() {
        String query = "SELECT " + ScoreDatabase.SCORE_USERNAME + ", MAX(" + ScoreDatabase.SCORE_SCORE + ") AS MAXSCORE, " + ScoreDatabase.SCORE_DATESCORE + " FROM " + ScoreDatabase.SCORE_TABLE + " GROUP BY " + ScoreDatabase.SCORE_USERNAME + " ORDER BY MAXSCORE DESC";
        Cursor cursor = scoreDatabase.getReadableDatabase().rawQuery(query, null);
        ...
        while (!cursor.isAfterLast()) {
            userList.add(new ScoreModel(cursor.getString(0), cursor.getInt(1), cursor.getString(2)));
            cursor.moveToNext();
        }
